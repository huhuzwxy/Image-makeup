...
# 信号槽绑定
    def action_connect(self):
        ...
        # 标记人眼位置
        self.ui.action_19.triggered.connect(self.mark_eyes)

        # 标记人脸关键点
        self.ui.action_20.triggered.connect(self.mark_points)
...
# 饱和度
    def change_saturation(self):
        ...
        image = Image.fromarray(cv2.cvtColor(self.current_img,cv2.COLOR_BGR2RGB))
        image = ImageEnhance.Color(image).enhance(value/10)
        self.current_img = cv2.cvtColor(np.asarray(image),cv2.COLOR_RGB2BGR)
...
# 亮度
    def change_darker(self):
        ...
        #value = self.ui.horizontalSlider.value()
        image = Image.fromarray(cv2.cvtColor(self.current_img, cv2.COLOR_BGR2RGB))
        image = ImageEnhance.Brightness(image).enhance(value/10)
...
# 人脸识别
    def detect_face(self):
        img = self.raw_image
        face_cascade = cv2.CascadeClassifier('/usr/local/opt/opencv@3/share/OpenCV/haarcascades/haarcascade_frontalface_default.xml')
...
# 眼睛识别
    def detect_eyes(self):
        img = self.raw_image
        eye_cascade = cv2.CascadeClassifier('/usr/local/opt/opencv@3/share/OpenCV/haarcascades/haarcascade_lefteye_2splits.xml')

        faces = self.faces
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        #eyes = []
        for (x, y, w, h) in faces:
            roi_gray = gray[y : y + int(float(h/2)), x : x + w]
            eyes = eye_cascade.detectMultiScale(roi_gray,1.2,2)
            #for (ex, ey, ew, eh) in result:
            #    eyes.append((x + ex, y + ey, x + ex + ew, y + ey + eh))
        return eyes
...
# 亮眼
    def bright_eyes(self, value=30):
        # value = 30
        value = self.ui.horizontalSlider_15.value()
        img = self.current_img
        ...
        for x in [ex, ex + ew]:
            for y in [ey, ey + eh]:
                cv2.filter2D(img[x, y], -1, kernel)
                img_hsv[x, y, 2] = np.log(img_hsv[x, y, 2] / 255 * (60 - 1) + 1) / np.log(60 + 1) * 255
                imgw = cv2.cvtColor(img_hsv, cv2.COLOR_HSV2BGR)
    ...
    def calculate(self):
        ...
        if self.calculated is False:
            ...
            if self.faces != ():
                ...
                self.points = self.detect_points()
                self.detect_skin()
...
# 铅笔画(灰度)
    def pencil_gray(self):
        ...
        dst1_gray, dst1_color = cv2.pencilSketch(self.current_img, sigma_s=50, sigma_r=value/50, shade_factor=0.04)
...
# 风格化
    def stylize(self):
        ...
        self.current_img = cv2.stylization(self.current_img, sigma_s=50, sigma_r=value/50)
...
# 细节增强
    def detail_enhance(self):
        ...
        self.current_img = cv2.detailEnhance(self.current_img, sigma_s=50, sigma_r=value/50)
...
# 边缘保持
    def edge_preserve(self):
        ...
        self.current_img = cv2.edgePreservingFilter(self.current_img, flags=1, sigma_s=50, sigma_r=value/50)
...
# 初始化
    def intial_value(self):
        ...
        self.ui.horizontalSlider_4.setValue(10)
...
# 显示人脸识别
    def mark_face(self):
        ...
        for (x, y, w, h) in self.faces:
            self.current_img = cv2.rectangle(self.current_img.copy(), (x, y), (x+w, y+h), (255, 0, 0), 2)
        self.show_image()
...
# 显示人脸关键点
    def mark_points(self):
        ...
        if self.calculated == False:
            self.calculate()
        for face_landmark in self.points:
            facial_features = ['chin', 'left_eyebrow', 'right_eyebrow', 'nose_bridge', 'nose_tip', 'left_eye',
            ...
            for facial_feature in facial_features:
                d.line(face_landmark[facial_feature], width=2)
            pil_img.show()
    ...
    def mark_eyes(self):
        ...
        for (x, y, w, h) in self.faces:
            for (ex, ey, ew, eh) in self.eyes:
                cv2.rectangle(self.current_img[y : y + int(float(h/2)), x : x + w],(ex, ey), (ex + ew, ey + eh),(0, 255, 0),2)
        self.show_image()
